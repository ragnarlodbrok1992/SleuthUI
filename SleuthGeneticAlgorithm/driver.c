  ...
  drv_driver ()
{
  ...
  if (proc_GetProcessingType () == PREDICTING)
  {
    ...
    printf("\ngdif_WriteGIF gif_filename = %s %s", filename, "124 driver.c");
    ...
    if (scen_GetDoingLanduseFlag ())
    {
      ...
      printf("\ngdif_WriteGIF gif_filename = %s %s", filename, "150 driver.c");
      ...
      colortable_ptr = color_GetColortable (GRAYSCALE_COLORTABLE);
      assert (colortable_ptr != NULL);
      printf("\ngdif_WriteGIF gif_filename = %s %s", filename, "164 driver.c");
